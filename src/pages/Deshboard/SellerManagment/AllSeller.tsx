...
const AllSeller: React.FC = () => {
  ...
  const totalPages = Math.ceil(initialSellers.length / itemsPerPage);
    ...
    <div className="bg-white p-6 rounded-2xl border border-gray-100  w-full font-sans">
        ...
        <table className="w-full text-left border-collapse">
          <thead className='  '>
            <tr className="bg-[#C1E0F6] border-y rounded-lg border-gray-100">
              <th className="p-4 font-bold text-gray-600 text-sm">Seller Name</th>
          ...
          <tbody>
            {currentSellers.map((seller, index) => (
              <tr key={index} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    <img src={seller.image} alt={seller.name} className="w-10 h-10 rounded-lg object-cover border border-gray-100 shadow-sm" />
                    <div>
                      <p className="text-gray-800 font-bold text-sm">{seller.name}</p>
                      <p className="text-gray-400 text-xs font-medium">{seller.id}</p>
                    </div>
                  </div>
                </td>
      ...
      <div className="flex items-center justify-between mt-8">
        <p className="text-sm text-gray-500 font-medium">
          Showing <span className="text-gray-800">{indexOfFirstItem + 1}-{Math.min(indexOfLastItem, initialSellers.length)}</span> of <span className="text-gray-800">{initialSellers.length}</span> sellers
        </p>
